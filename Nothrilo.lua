...
local UI_TITLE = MENU_NAME .. " | By Cafezl"
...
local function applyStabilizer(cart)
    ...
    if #wheels == 0 then
        notify("Stabilizer", "Nenhuma roda foi encontrada.")
        return
    end
...
local function refreshStabilizer()
    if stabilizer.enabled then
        ...
        if cart then
            ...
            notify("Stabilizer", "Sente em um carrinho primeiro.")
        end
    else
        cleanupStabilizer()
    end
...
local function sitOnVehicleSeat(seat)
    ...
    -- tentativas curtas tornam o comando mais confiavel que uma so tentativa.
    for _ = 1, 3 do
...
local function executeKiller(partialName)
    ...
    if not target or not target.Character then
        notify("Killer", "Jogador não encontrado.")
        return
    end

    local targetRoot = getRoot(target.Character)
    if not targetRoot then
        notify("Killer", "O alvo não tem HumanoidRootPart.")
        return
    end
    ...
    if not seat then
        notify("Killer", "Não há carrinho disponível.")
        return
    end
    if not sitOnVehicleSeat(seat) then
        notify("Killer", "Não foi possível sentar no carrinho.")
        return
    end
...
PlayerSection:NewSlider("WalkSpeed", "Define a velocidade do personagem.", 500, 0, function(value)
    local humanoid = getHumanoid()
    if humanoid then
...
PlayerSection:NewButton("Reset WalkSpeed", "Volta a velocidade para 16.", function()
    local humanoid = getHumanoid()
    if humanoid then
...
PlayerSection:NewToggle("Infinity Jump", "Permite pular enquanto estiver no ar.", function(state)
    infiniteJump = state
end)
...
PlayerSection:NewButton("Click Teleport", "Cria a ferramenta ClickTP na mochila.", function()
    local backpack = LocalPlayer:FindFirstChildOfClass("Backpack")
    if not backpack then return end
    if backpack:FindFirstChild("ClickTP") then
        notify("ClickTP", "A ferramenta já está na mochila.")
        return
    end
...
PlayerSection:NewTextBox("Goto", "Digite o nome do jogador e aperte Enter.", function(name)
...
local flyToggle = PlayerSection:NewToggle("Vehicle Fly", "WASD move; Q desce e E sobe.", function(state)
    flyEnabled = state
    if state then
        startVehicleFly()
    else
...
PlayerSection:NewTextBox("VFly Speed", "Digite a velocidade e aperte Enter.", function(value)
    ...
    if number and number > 0 then
        ...
        notify("Vehicle Fly", "Velocidade: " .. number)
    else
        notify("Vehicle Fly", "Digite um número maior que zero.")
    end
end)
...
TeleportSection:NewButton("Secret Badge", "Teleporta para a Secret Badge.", function()
    teleportCharacter(CFrame.new(234.500259, 2.28650475, 296.495483))
end)

TeleportSection:NewButton("Cart Spawn Button", "Teleporta para o botão de carrinho.", function()
    teleportCharacter(CFrame.new(-33, 3.11, 21.5) * CFrame.Angles(0, math.rad(180), 0))
end)

TeleportSection:NewButton("TP to Suffering Team", "Teleporta para o time Suffering.", function()
    teleportCharacter(CFrame.new(-416.844727, 163.402969, 171.087555))
end)

TeleportSection:NewButton("TP to Secret Room", "Procura Workspace.Misc.Giver.", function()
...
TeleportSection:NewButton("Auto Win", "Executa a rota de teste do mapa.", function()
    task.spawn(function()
        if not teleportCharacter(CFrame.new(-430.898926, 165.25, 101.645676)) then return end
        task.wait(4.1)
        teleportCharacter(CFrame.new(-430.898529, 163.273926, 131.145554))
        task.wait(10)
        local humanoid = getHumanoid()
        ...
        task.wait(2)
        teleportCharacter(CFrame.new(-500.748535, -21.78265, -302.303406))
        task.wait(0.1)
        local root = getRoot()
        ...
        task.wait(0.6)
        teleportCharacter(CFrame.new(-470.360138, -21.1945782, -302.303101))
        task.wait(10)
        ...
        if humanoid then humanoid.Sit = false end
        task.wait(0.5)
        teleportCharacter(CFrame.new(-421.770264, -44.2260475, -297.081909))
    end)
end)

local CartTab = Window:NewTab("Cart")
local CartSection = CartTab:NewSection("Cart Control")

CartSection:NewToggle("Panic Stop", "Para e trava o carrinho; desligue para restaurar.", setPanicStop)

CartSection:NewButton("Teleport to Checkpoint 1", "Move o carrinho para o checkpoint 1.", function()
    teleportCart(CFrame.new(-430.898926, 164.75, 101.645676) * CFrame.Angles(0, math.rad(90), 0))
end)

CartSection:NewButton("Teleport to Checkpoint 2", "Move o carrinho para o checkpoint 2.", function()
    teleportCart(CFrame.new(511.88, 3.69, 306.59) * CFrame.Angles(0, math.rad(270), 0))
end)

CartSection:NewButton("Teleport to Checkpoint 3", "Move o carrinho para o checkpoint 3.", function()
    teleportCart(CFrame.new(171.09, 2.78, -410.31) * CFrame.Angles(0, math.rad(90), 0))
end)

local StabilizerSection = CartTab:NewSection("Stabilizer")
StabilizerSection:NewToggle("Cart Stabilizer", "Mantém o carrinho estável enquanto você está sentado.", function(state)
    stabilizer.enabled = state
    refreshStabilizer()
end)

StabilizerSection:NewTextBox("Stabilizer Force", "Força normal; aperte Enter para aplicar.", function(value)
    local number = tonumber(value)
    if number and number > 0 then
        STABILIZER_CONFIG.NORMAL_FORCE = number
        notify("Stabilizer", "Força normal: " .. number)
    else
        notify("Stabilizer", "Digite um número maior que zero.")
    end
end)

StabilizerSection:NewTextBox("Downhill Force", "Força em descidas; aperte Enter para aplicar.", function(value)
    local number = tonumber(value)
    if number and number > 0 then
        STABILIZER_CONFIG.DOWNHILL_FORCE = number
        notify("Stabilizer", "Força de descida: " .. number)
    else
        notify("Stabilizer", "Digite um número maior que zero.")
    end
end)
...
local KillerSection = KillerTab:NewSection("Target Control")
local targetName = ""

KillerSection:NewTextBox("Target Name", "Digite uma parte do nome e aperte Enter.", function(value)
    targetName = value
end)

KillerSection:NewButton("Kill Target", "Usa o carrinho livre mais próximo para alcançar o alvo.", function()
    if targetName == "" then
        notify("Killer", "Digite uma parte do nome primeiro.")
        return
    end
    task.spawn(executeKiller, targetName)
end)
...
local running = true
task.spawn(function()
    while running and menuGui.Parent and launcherGui.Parent do
...
notify(MENU_NAME, "By Cafezl  •  K minimiza e reabre o menu.")
