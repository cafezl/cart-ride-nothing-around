...
local function finishKiller(message)
    ...
    if message then
        notify("Killer", message)
    end
end

local function executeKiller(partialName)
    if killerActive then
        notify("Killer", "Aguarde a tentativa atual terminar.")
        return
    end
    killerActive = true
...
GuiSection:NewButton("Destroy Menu", "Fecha tudo do Nothrilo com seguranca.", destroyNothrilo)
GuiSection:NewKeybind("Destroy Menu", "Tecla X fecha tudo do Nothrilo.", Enum.KeyCode.X, destroyNothrilo)
